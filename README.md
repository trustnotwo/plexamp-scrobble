...
Go to https://www.last.fm/api/account/create (log in with your Last.fm account)
Application name: anything, e.g. Plex Scrobbler. Leave the other fields blank.
Submit. Copy the API Key and Shared Secret somewhere — you need them in Step 3.
...
bashmkdir -p /mnt/NVME/apps/tautulli/scripts
...
bashecho -n "YourLastfmUsername"  > /mnt/NVME/apps/tautulli/secrets/lastfm_username.txt
...
bashchmod 600 /mnt/NVME/apps/tautulli/secrets/*.txt
...
bashcp scrobble_plex.py /mnt/NVME/apps/tautulli/scripts/
...
bashdocker exec -it tautulli python3 /config/scripts/scrobble_plex.py "start" "Test Artist" "Test Album" "Test Track" 240 0
...
SettingValueScript Folder/config/scriptsScript File./scrobble_plex.pyScript Timeout30DescriptionScrobble Plex
...
ParameterOperatorValueMedia Typeistrack
...
Tautulli → gear icon → View Logs → Notification Logs shows the script ran# Tautulli → Last.fm Scrobbler (`scrobble_plex.py`) — Setup Guide
